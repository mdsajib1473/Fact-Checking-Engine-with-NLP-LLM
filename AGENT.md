...
**Phase 4 — Completed:** Verdict engine + UI. `verdict_engine.generate_verdict(claim, evidence, lang)` is the sole public entry point: sends claim + numbered evidence to the **Groq chat-completions API (`openai/gpt-oss-120b` — `llama-3.3-70b-versatile` is deprecated)** with a chain-of-thought judge prompt and a strict JSON output contract, then parses/validates the reply (label whitelist, confidence clamped 0–10). **Empty evidence short-circuits to UNVERIFIABLE without any LLM call** (Rule 9 + saves quota); the prompt additionally forbids outside knowledge and mandates UNVERIFIABLE when evidence is off-point. Every failure path (missing key, timeout, rate limit, malformed reply) returns the UNVERIFIABLE fallback — never a crash (Rule 15). The exact Rule 3 disclaimer is appended to every verdict, with a Bangla translation for `bn` claims (never mixed). `claim_service.score_claim()` persists the `Verdict` row (label stored lowercase per model choices) and links the Phase-3 `Source` rows to it (`verdict_id` updated from NULL); Verdict+Source rows with `created_at` form the Rule 14 audit trail. `POST /api/v1/extract/` now returns `verdict` per claim; new `POST /api/v1/check-url/` scrapes a page (Phase 3 `scrape_url`) and runs the same pipeline with `source_type="url"` + `source_url` recorded. **UI:** home tab switcher wired (vanilla `fetch`, client-side validation mirroring API bounds, CSRF header, loading spinner); report page renders client-side from `sessionStorage` (`report_interactions.js`) — color-coded verdict badges (green/yellow/red/gray), confidence bar, explanation, always-visible disclaimer, expandable per-claim evidence panels (`<details>`) with clickable source links + relevance scores (Rule 10), simple-view toggle (hides explanation+evidence only), dark mode works, all DOM built via `textContent` (no innerHTML injection). Language switcher now functional: a small per-language string dictionary in `base.html` (`data-i18n` attributes + `window.FC_I18N`), EN/বাংলা, chrome-only (content keeps its detected language). 15 new tests (9 verdict engine, all Groq HTTP mocked; 6 API integration/URL) — **62 total pass** via `manage.py test factcheck.tests --settings=core.settings_test`; `manage.py check` clean. Manually verified in the browser: English paragraph → 2 SUPPORTED claims (10/10, 9/10) with evidence panels; Bangla claim → Bangla explanation + exact Bangla disclaimer + `bn.wikipedia.org` evidence + Bangla chrome; dark mode and simple view both correct.

**Phase 4 decisions / notes:**

- **Groq via plain `requests`** against the OpenAI-compatible `chat/completions` endpoint — no new SDK dependency. `temperature=0`, `response_format={"type":"json_object"}`, model/key/URL/timeout/max-tokens all from settings (`GROQ_MODEL=openai/gpt-oss-120b`, `GROQ_TIMEOUT_SECONDS=30`).
...
- **Report data flow:** the API response is stored in `sessionStorage` and rendered client-side on `/report/` — no server-side report persistence needed this phase (report-by-id permalinks can come later if wanted).
...
- **Observed:** the zero-shot Bangla extractor is conservative — "আইফেল টাওয়ার ফ্রান্সের রাজধানী প্যারিসে অবস্থিত।" scored below the claim threshold and produced an empty (graceful) report, while "ঢাকা বাংলাদেশের রাজধানী।" passed and verified end-to-end. Phase 2 tuning question, out of Phase 4 scope.
- Browser verification wrote a handful of Eiffel/Curie/ঢাকা rows (claims/verdicts/sources) to Neon — left in place, same as the Phase 3 test rows.

**Phase 5 — Completed (project feature-complete, pending manual deployment):** Security, logging, deployment readiness. **Rate limiting:** DRF `ScopedRateThrottle` (scope `factcheck`) on `/api/v1/extract/` and `/api/v1/check-url/` — one combined per-client budget, `RATE_LIMIT_PER_HOUR` from settings (default 10, Rule 7), 429 + `Retry-After` beyond it; verified live (10 rapid requests pass, 11th → 429 with "Request was throttled. Expected available in 3600 seconds."). **CSRF:** DRF `SessionAuthentication` enforces CSRF for session-authenticated requests (the actual CSRF attack surface — anonymous requests carry no ambient credentials); the frontend sends `X-CSRFToken` on every fetch regardless. Content-type robustness: malformed JSON → 400, unexpected content type → 415, wrong field types → 400 — never 500 (tested). **Prompt-injection hardening (gap found + fixed):** claim + evidence text (user-influenced via input, scraped pages, third-party APIs) is now wrapped in `BEGIN DATA`/`END DATA` markers in the Groq user turn, and the system prompt declares marked content untrusted data whose embedded instructions must be ignored. Sanitization chain re-verified end-to-end: extraction strips HTML/scripts (Phase 2), evidence `_sanitize` + scraper tag-strip/cap (Phase 3), report UI renders via `textContent` only (Phase 4). **Secrets audit:** scanned all 56 git-tracked files for the *actual values* of `DJANGO_SECRET_KEY`, `NEON_DATABASE_URL`, `GROQ_API_KEY`, `GOOGLE_FACTCHECK_API_KEY` — zero leaks; `.env` untracked; boot-refusal without `DJANGO_SECRET_KEY` under `DEBUG=False` re-verified. **Audit trail (Rule 14):** Verdict (claim FK, label, score, explanation, `created_at`) + linked Source rows confirmed complete; admin registrations made strictly read-only for Claim/Verdict/Source/ClaimEmbedding (add/change/delete disabled — immutable trail), CacheEntry stays deletable (operational). **DEBUG=False smoke test:** home + static 200 via WhiteNoise, 404 renders a plain page with no stack trace, `SECURE_SSL_REDIRECT` 301s http→https (bypassed in the test via `X-Forwarded-Proto: https`, exactly how Render's proxy presents requests); `collectstatic` clean (433 post-processed). **Deployment files:** `build.sh` now downloads `en_core_web_sm` and pre-caches the MiniLM relevance model at build time (skipped when `ENABLE_EMBEDDING_RELEVANCE=0`); `render.yaml` adds `GROQ_MODEL`, `ENABLE_HF_FALLBACK=0`, `ENABLE_EMBEDDING_RELEVANCE=1`, `RATE_LIMIT_PER_HOUR=10`, and a tuned start command `gunicorn --workers 1 --threads 4 --timeout 120`. 7 new security tests — **69 total pass** (`--settings=core.settings_test`), `manage.py check` clean. Not committed/deployed (Rule 4).

**Phase 5 decisions / notes:**

- **512 MB memory verdict:** Django+gunicorn (~100 MB) + spaCy `en_core_web_sm` (~60 MB) + torch runtime + MiniLM (~150 MB) fits with one worker; the Bangla zero-shot model (mDeBERTa, ~700 MB+ loaded) **cannot** fit, so `ENABLE_HF_FALLBACK=0` on Render — Bangla claim extraction degrades to the Phase 2 heuristic pass (English unaffected; it never used the transformer). If the service still OOMs under real traffic, set `ENABLE_EMBEDDING_RELEVANCE=0` next (relevance falls back to lexical overlap). Lazy-loading confirmed truly lazy: models load on first use only, never at boot.
- **`--timeout 120` on gunicorn:** a cold pipeline request (spaCy load + 3 evidence sources + Groq) can exceed the 30 s default, which would kill the worker mid-request.
- **Throttle testing quirk:** DRF binds `THROTTLE_RATES` at import, so tests patch `ScopedRateThrottle.THROTTLE_RATES` directly (`override_settings(REST_FRAMEWORK=...)` doesn't propagate); `core/settings_test.py` sets a high rate so the rest of the suite never trips the limiter (throttle history lives in the process-wide cache).
...
- [x] Decide on rate-limit thresholds per IP (placeholder: 10 requests/hour for unauthenticated users) *(Phase 5)*
      *(Kept 10/hour as the default, configurable via `RATE_LIMIT_PER_HOUR`; both pipeline endpoints share one budget per client.)*
